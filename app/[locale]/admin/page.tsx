...
const PageContainer = dynamic(() => import('@/components/admin/PageContainer.tsx'), { ssr: false });
import DashboardCard from '@/components/admin/DashboardCard.tsx';
import ReferenceUploader from '@/components/admin/ReferenceUploader.tsx';
  ...
  const saveContent = async () => {
    try {
      ...
      const contentToSave = {
        ...
        previewImages: contentData[mainLanguage].previewImages, // 确保预览图像被正确保存
        references: files, // 假设您有一个字段来保存参考图像
      };
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      reader.onloadend = async () => {
        const base64File = reader.result?.toString().split(',')[1]; // 获取Base64编码
        if (base64File) {
          try {
            ...
            if (response.ok) {
              const { url } = await response.json(); // 获取返回的URL
              // 更新预览图像数组
              updateField('previewImages', [...contentData[languages[currentLang]].previewImages, url]);
              console.log('上传成功，图片URL:', url); // 打印返回的URL
            } else {
              const errorData = await response.json();
              console.error('上传失败:', errorData.error);
            }
          } catch (error) {

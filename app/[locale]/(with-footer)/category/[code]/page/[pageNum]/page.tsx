...
export async function generateMetadata({ params }: { params: { code: string; pageNum?: string; locale: string } }): Promise<Metadata> {
...
export default async function Page({ params }: { params: { code: string; pageNum?: string; locale: string } }) {
  const supabase = createClient();
  const { t: _t, ...otherProps } = getTranslations('Category');
